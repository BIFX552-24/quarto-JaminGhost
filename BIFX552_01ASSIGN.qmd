...
**NOTE:** This page is copied from RPubs: [An introduction to R using iris](https://rpubs.com/moeransm/intro-iris) for learning purposes.
...
# For a full list of these datasets, typelibrary(help = "datasets") 
```
...
Written packages make it easier to work with datasets than regular baseR functions. They have been optimized to be faster and more intuitive than baseR functions, therefore reducing the steepness of the R learning curve. Let’s take a look;{r}
...
# select all columns from sepal.length to petal.length selected2 <- select(iris, sepal.length:petal.length) head(selected, 3)
```

```         
# selected and selected2 are exactly the same identical(selected, selected2)
```
